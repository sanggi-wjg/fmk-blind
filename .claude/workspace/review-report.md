...
## 리뷰 #6 — 모바일 지원(TODO Q2) — 2026-07-08 · 브랜치 `feat/mobile-support` — **MERGE (blocker 0 / major 0 / minor 2 + nit)**

- 방법론: `extension-code-review` 스킬 6차원. 대상: `git diff main` 전체(10파일, +143/−15).
- 상보성: extension-qa PASS(12/12 — 실측 하네스·라이브 curl·CSS 정합·문서 정합, qa-report.md 최신 섹션). QA 검증분(계약·셀렉터 실측·매치패턴 유효성)은 재지적하지 않고 **코드 품질·문서 드리프트**에 집중.
- 변경 성격: **실행 JS 무변경**(20-selectors.js는 주석만 — node --check OK, 공백·주석 제거 후 main↔branch 실행코드 동일). manifest `matches`에 `https://m.fmkorea.com/*` 추가·ver 0.6.0, content.css·popup.css `@media (pointer: coarse)` 가산, 문서(SKILL/PLAN/README/TODO/DEPLOY/qa) 갱신.

### 차원별 판정
- **정확성 — PASS.** 실행 로직 무변경(주석만). 신규 주석의 기술 주장을 코드와 대조 검증: ①20-selectors 규칙 ② 조상 체인 주석(`.rd_hd`→`.rd`)이 실제 `findContainer` `closest('.rd_hd, .top_area')`→`closest('.rd')`와 일치. ②content.css 44px 계산 주석(15px×1.2≈18px + 상하패딩 26px=44px)이 실제 `padding:13px 18px`(수직 26px)과 정확히 맞음. ③popup.css "데스크톱 360px 고정 그대로" 주석이 base `body{width:360px}`(:27)·`max-height:560px`(:29)와 일치, override의 `max-height:none`이 이를 정상 해제. 신규 CSS가 겨냥하는 셀렉터(`#fmkb-context-menu`/`.fmkb-menu-item`/`#fmkb-toast`/`.fmkb-search`/`.fmkb-unblock`/`.fmkb-io-btn`) 전부 JS·HTML에 실존(grep 확인). 매치패턴 2종 MV3 문법 유효(정규식 실측).
- **보안/최소권한 — PASS.** `permissions:["storage"]` 단독 유지, `host_permissions`·`scripting`·`tabs` 여전히 없음(manifest 파싱 확인). `matches`에 `m.fmkorea.com` 추가는 **같은 storage-only 권한 하에서 content script 주입 호스트를 동일 사이트 패밀리의 모바일 서브도메인으로만 확장** — 신규 권한 표면·`<all_urls>` 없음, 최소권한 그대로. 신규 코드 경로(=CSS만) → XSS/DOM 인젝션 싱크 추가 0.
- **견고성 — PASS(데스크톱 회귀 0).** 신규 CSS 규칙 전부 `@media (pointer: coarse)` 내부 → 데스크톱 base 규칙(메뉴 150px/`8px 14px`/13px, 토스트 `10px 18px`/13px, 팝업 body 360px/560px) **불변**. override는 동일 셀렉터·동일 특이도이고 파일 뒤쪽에 위치 → 미디어쿼리 매치 시에만 승리(데스크톱에선 미적용). popup.html에 `<meta viewport width=device-width>` 존재 → 모바일 `width:100%`가 실디바이스 폭 기준으로 정상 계산. **JS에 호스트 게이팅 없음**(`location.hostname`/`www.` 하드코딩 grep 0) → content script는 manifest matches로만 주입되므로 `m.` 추가만으로 충분, 서브도메인 배제 위험 없음.
- **MV3/크로스브라우저 — PASS.** `browser_specific_settings.gecko`(id `raynor-back@proton.me` + `strict_min_version 115.0`) 존재 → Firefox(Android 포함) gecko 설정으로 로드, `gecko_android` 선택(부재 시 gecko 폴백). version 0.6.0 범프(release.yml 정합).
- **유지보수성 — PASS.** 20-selectors 단일 출처 원칙 유지(주석이 `fmk-dom-selectors` 스킬 참조, 스킬·PLAN·코드 주석이 규칙 ② 조상 체인을 일관 기술). 죽은 코드·매직넘버 신규 0.
- **문서 드리프트 — 대부분 정정, 잔여 minor 2.** 우려 대상인 라이브 "모바일 미지원"/"member_ 없음" 주장은 **전수 grep 결과 완전 purge**(잔존은 SKILL.md:56·PLAN.md:46의 명시적 "정정 이력" 노트 내부뿐 — 정상). 단 **범위 축소 문자열 `(PC)`가 3곳 잔존**(아래 MINOR-M1/M2).

### 발견
| ID | 심각도 | 차원 | 위치 | 요지 |
|----|--------|------|------|------|
| MINOR-M1 | minor | 문서드리프트(user-facing) | `manifest.json:5` | description `"에펨코리아(PC) 특정 유저 블라인드 확장"` — 이 PR이 같은 파일을 0.6.0로 범프하고 `m.` 매치를 추가했는데 설명은 여전히 `(PC)`로 범위를 축소. chrome://extensions·Firefox 부가기능 관리자·(배포 시)스토어에 노출되는 사용자 대면 문자열 → README/PLAN Q2/TODO/DEPLOY는 "PC+모바일"로 갱신됐으나 이 한 곳만 누락. **수정안:** `"에펨코리아(PC·모바일) 특정 유저 블라인드 확장"`. |
| MINOR-M2 | minor | 문서드리프트(내부 불일치) | `PLAN.md:3` | PLAN 최상단 요약 한 줄이 `에펨코리아(PC) 특정 유저 블라인드(차단) 크롬 확장.` — 같은 PLAN.md의 Q2(:43)·모바일 절(:45–51)은 "PC+모바일"로 갱신됨. 동일 파일 내 상단↔본문 불일치. **수정안:** 상단 한 줄도 `(PC·모바일)`로. |
| NIT-M3 | nit | 문서하이진 | `.claude/agents/content-engineer.md:9`, `CLAUDE.md:1` + 변경이력表 | 에이전트 도메인 설명 `fmkorea(PC)`·CLAUDE 헤더 `(fmkorea, PC)`가 모바일 커버 반영 안 됨(하네스 내부 문서, 저우선). CLAUDE.md 변경이력表에 2026-07-08 모바일 행 미기록 — CLAUDE.md 자체의 "문서 최신성" 규칙상 커밋 시 행 추가 권장. |

### 범위 밖 관찰(이번 diff 아님 — 인지용, 이 PR 판정 무관)
- `src/popup/popup.js:30` 주석 "onChanged는 TODO"는 **stale**(onChanged는 2026-06-15 구현·배선 완료). 같은 파일 `:510` 주석은 onChanged 반영을 전제로 최신 → popup.js 내부 주석 불일치. **선재 드리프트**(popup.js는 이번 diff 미포함).
- 본 `review-report.md:146`의 "미구현·결함 아님" 목록에 `모바일(m.fmkorea.com)`·onChanged·MutationObserver·내보내기/가져오기가 남아 있으나 **모두 구현 완료** — 리뷰 #1/#2 시점 스냅샷의 선재 staleness(감사 이력 문자열, 코드 결함 아님).

### 미검토(통과 처리 금지) · 실기기/모바일-UA 게이트 (QA와 동일)
- G1 지연 렌더(LazyFilter) 댓글이 `li#comment_{srl}.fdb_itm`로 렌더돼 MutationObserver가 숨기는지 · G2 Android 롱프레스 `contextmenu` 발화 + `preventDefault` 네이티브 메뉴 억제 · G3 롱프레스 직후 스크롤 경합 · G4 Firefox Android 팝업/반응형 · G5 서명 xpi/`web-ext run` 실로드(strict_min_version 115).

### 종합 판정: **MERGE (머지 가능)**
정적 검증 6차원에서 정확성·보안·견고성·MV3·유지보수성 **결함 0**. 실행 JS 무변경(주석·CSS·manifest 매치·버전만)이라 데스크톱 회귀 위험 없음, 최소권한 유지, 모든 신규 CSS는 터치 기기로 격리. blocker/major 0. 잔여 minor 2(MINOR-M1/M2)는 **문서 완결성**(누락된 `(PC)` 범위 문자열)뿐으로 머지를 막지 않으나, CLAUDE.md 문서 최신성 규칙상 **같은 작업 단위에서 manifest description + PLAN 상단 한 줄을 함께 정정**하면 드리프트를 완전히 닫는다(권장, 저비용). 모바일 런타임은 실기기 게이트(G1–G5)로 명시 잔존 — 이 프로젝트 원칙상 머지 전 사용자 실브라우저 확인.
