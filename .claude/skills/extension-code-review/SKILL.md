...
- 생명주기: `block`/`unblock` 즉시 영속화가 단수명 컨텍스트(팝업)에서도 유실 없는가. **언로드 자동 flush(`pagehide`/`visibilitychange`)는 두지 않는다** — stale 탭 resurrection 원인이라 제거됨(다시 있으면 결함). onChanged 핸들러는 읽기 전용(쓰기 없음)인가
...
## 범위 구분 (중요)
- **`TODO.md`의 v1 의도적 제외 항목**(MutationObserver/모바일/압축/내보내기 등)은 **결함이 아니다**. "미구현 = 범위 밖"으로 분류하고 결함으로 올리지 않는다. (단 **onChanged 라이브 동기는 2026-06-15 구현됨** — 더 이상 제외 항목 아님.)
