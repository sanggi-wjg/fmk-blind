...
검증 그린(Phase 4: QA blocker/major 0 + 리뷰 MERGE)·팀 정리(Phase 5) 후, Phase 0.5에서 브랜치를 땄다면:
1. **검증 게이트**: 저장 계층·다중 컨텍스트(onChanged 등) 변경은 mock/로직 테스트만으로 PASS 단정 금지 — **실브라우저가 최종 게이트**(거짓 PASS 전례, [[storage-verify-in-real-browser]]). 고위험 변경은 PR 생성 전 사용자 실브라우저 확인을 권장하거나, PR 본문 체크리스트에 "실브라우저 검증"을 미결로 명시.
2. **버전 범프 정책**: 릴리즈가 필요한 변경이면 `manifest.json`의 `version`을 올린다(semver). `release.yml`이 버전 기준으로 릴리즈를 만들므로 **안 올리면 머지해도 릴리즈가 안 생긴다**. 문서/내부 산출물만 바뀌면 생략 가능.
3. **커밋**: `git add -A`(`.gitignore`로 zip·`settings.local.json` 제외 확인) → 한국어 요약 + 본문(변경·검증 결과). 메시지 끝에 `Co-Authored-By: Claude Opus 4.8 (1M context) <noreply@anthropic.com>`.
4. **푸시**: `git push -u origin <branch>`.
5. **PR 생성**: `gh pr create --base main --head <branch>` + 템플릿 본문 — `## 요약` / `## 변경` / `## 검증`(QA·리뷰·테스트·실브라우저) / `## 머지 전 확인`(저장소 Actions 쓰기 권한, 버전 범프). 본문 끝에 `🤖 Generated with [Claude Code](https://claude.com/claude-code)`.
6. **PR에서 정지** — 자동 머지 금지(머지는 사용자 몫). 머지 시 `release.yml`이 이어받아 `v{version}` 릴리즈 + zip을 만든다.
7. PR URL을 사용자에게 보고.
