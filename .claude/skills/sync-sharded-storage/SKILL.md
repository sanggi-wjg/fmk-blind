...
description: "FMK-Blind 차단 목록의 chrome.storage.sync 샤딩 저장 계층 설계와 store.js API 계약. 메모리 맵, bl_meta 버전, bl_0..N 청크 분할(8KB/100KB 제약), 디바운스 쓰기, stale 청크 정리, block/unblock/isBlocked/list/count API를 정의. 저장·동기화·샤딩·차단목록 영속화·store API 작업 시 반드시 이 스킬을 사용할 것."
---
...
- 쓰기: 분당 120회, 시간당 1,800회. → 쓰기는 **디바운스**하고 변경된 청크만 기록.
...
const store = {
  ...
  async block(uid, nick),  // 추가 + 디바운스 영속화. addedAt = 호출 시각
  async unblock(uid),      // 제거 + 디바운스 영속화
...
- 영속화는 디바운스(예: 500ms)로 묶되, `unblock`/`block` 직후 메모리 상태는 즉시 반영되어 `isBlocked`가 곧바로 정확해야 한다.
